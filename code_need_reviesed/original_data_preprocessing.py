...
def get_data(
        params: Dict,
        url=f'http://{host}:{port}/tqmain/equity_daily',  # 默认获取股票日度表
        res=None
) -> pd.DataFrame:
    '''
    主要获取DB中的数据
...
def getting_filter_data(start_date, end_date, url=f'http://{host}:{port}/tqmain/equity_daily', wind_code=None):
    params = {}
    ...
    return df
    # result = pd.concat([getting_filter_data(start_date=trading_list[i], end_date=trading_list[i]) for i in range(0, len(trading_list))], axis=0, ignore_index=True)
    # result.to_csv("filter_data_table.csv", header=True, index=True)
    # print("Finish!")


def getting_original_data(start_date, end_date, url=f'http://{host}:{port}/tqmain/equity_daily', wind_code=None):
    params = {}
    params['wind_code'] = wind_code
    params['start_date'] = start_date
    params['end_date'] = end_date
    ...
    df = get_data(params=params, url=url)
    return df
    # result = pd.concat([getting_original_data(start_date=trading_date[i], end_date=trading_date[i]) for i in range(0, len(trading_date))], axis=0, ignore_index=True)
    # result.to_csv("original_data_table.csv", header=True, index=True)
    # print("Finish!")


def getting_y_data(start_date, end_date, url=f'http://{host}:{port}/tqmain/equity_daily', wind_code=None):
    params = {}
    params['wind_code'] = wind_code
    params['start_date'] = start_date
    params['end_date'] = end_date
    ...
    df = get_data(params=params, url=url)
    return df
    # result = pd.concat([getting_y_data(start_date=trading_date[i], end_date=trading_date[i]) for i in range(0, len(trading_date))], axis=0, ignore_index=True)
    # result.to_csv("original_y_data_table.csv", header=True, index=True)
    # print("Finish!")


def getting_equity_header_data(delist_date=None, url=f'http://{host}:{port}/tqmain/equity_header', wind_code=None):
    params = {}
    params['wind_code'] = wind_code
    ...
    df = get_data(params=params, url=url)
    return df
    # result = getting_equity_header_data()
    # result.to_csv("delist_date_table.csv", header=True, index=True)
    # print("Finish!")
