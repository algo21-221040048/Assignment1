...
def calculate_return1(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_new['return1'] = (df_new.groupby('wind_code'))['close_adj'].pct_change().reset_index(drop=True)
...
def calculate_return_bn(df: pd.DataFrame) -> pd.DataFrame:
    df_new = df.copy()
    df_new['return5'] = (df_new.groupby('wind_code'))['close_adj'].pct_change(periods=PERIOD).reset_index(drop=True)
...
# This function is used to filter the NaN in return1 and return5 group by wind_code
...
if __name__ == '__main__':
    ...
    #
    #             # Split x
    #             split_sub_df_x = f.unfold(sub_df_x.unsqueeze(0).unsqueeze(0), kernel_size=(9, 30), stride=(1, CAI_YANG_FREQUENCY))
    ...
    #
    #             # Handle y
    #             sub_df_y = np.array(sub_df.iloc[29::CAI_YANG_FREQUENCY]['return5'])
